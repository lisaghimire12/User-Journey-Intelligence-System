...
Registration-based intervention analysis is excluded because the
real dataset does not contain registration events.
"""
...
def build_recommendations(
    ...
    # ------------------------------------------------------------
    # MERGE AVAILABLE CAUSAL PARAMETERS
    # ------------------------------------------------------------
    ...
    for key, meta in intervention_engine.INTERVENTIONS.items():
        ...
        if (
            q_id
            and q_id in causal_question_map
            and not causal_df.empty
        ):
        ...
        if scenario_key:
            ...
            baseline = simulation_engine.run_simulation(
                simulation_engine.SimulationParams(
                    **{
                        **baseline_params.__dict__,
                        "n_sessions": n_sim_sessions,
                    }
                )
            )
            ...
            scenario_result = simulation_engine.run_simulation(
                simulation_engine.SimulationParams(
                    **{
                        **scenario_params.__dict__,
                        "n_sessions": n_sim_sessions,
                    }
                )
            )

            simulated_improvement_pct = round(
                scenario_result["conversion_rate"]
                - baseline["conversion_rate"],
                2,
            )

            if causal_effect_pct is not None:
                uncertainty = min(
                    uncertainty,
                    scenario_result["uncertainty"],
                )
            else:
                uncertainty = scenario_result["uncertainty"]
        ...
        affected_pct = (
            intervention_engine.affected_session_share(
                journeys_df,
                meta["affected_stage_pages"],
            )
        )
        ...
        benefit_signal = (
            causal_effect_pct
            if causal_effect_pct is not None
            else simulated_improvement_pct
        )

        benefit_signal = (
            benefit_signal
            if benefit_signal is not None
            else 0.0
        )
        ...
        penalty = (
            COMPLEXITY_PENALTY[meta["complexity"]]
            + RISK_PENALTY[meta["risk"]]
            + (uncertainty * 0.15)
        )
    ...
    return sorted(
        scores,
        key=lambda s: s.recommendation_score,
        reverse=True,
    )